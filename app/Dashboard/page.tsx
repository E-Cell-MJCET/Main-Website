        ...
        try {
          ...
          if (sessionData) {
            ...
            if (session && session.expiry > now) {
              ...
              const normalizedPermittedEmails = permittedEmails.map(e => e.trim().toLowerCase());
...
return;
  ...
  const handleRegistrationComplete = async (success: boolean, message: string) => {
              ...
              {isSignedIn && (
                <div className="mb-6 space-y-6">
                  ...
                  <div className="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                    <h3 className="mb-3 text-lg font-medium">Account Information</h3>
                    <div className="space-y-2">
                      ...
                      {loginLocation && (
                        <div className="flex items-center justify-between">
                          <span className="text-gray-600 dark:text-gray-400">Login Location:</span>
                        ...
                        <div className="flex items-center">
                          {isPermittedMember ? (
                            <>
                              ...
                              <span className="text-blue-600 dark:text-blue-400">Permitted Member</span>
                            </>
                          ) : (
                            <>
                              <AlertCircle className="mr-1 size-4 text-red-500" />
                              <span className="text-red-600 dark:text-red-400">Not a Member</span>
                            </>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Sidebar Default</span>
                      <button
                        onClick={() => {
                          if (!isMobile) {
                            setSidebarOpen(!sidebarOpen);
                  ...
                  <div className="mt-8 border-t border-gray-200 pt-6 dark:border-gray-700">
                    ...
                    <div className="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                      <div className="space-y-4">
                        ...
                        <div>
                          <label htmlFor="memberType" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Member Type
                          </label>
                          <select
                            id="memberType"
                    ...
                    <div className="mt-6">
                      <h4 className="text-md mb-2 font-medium">Current Permitted Members</h4>
                            ...
                            {permittedEmails.map((email, index) => (
                              <tr key={index}>
                                <td className="whitespace-nowrap px-6 py-4">
                                  <div className="text-sm text-gray-900 dark:text-gray-200">{email}</div>
                                </td>
                                <td className="whitespace-nowrap px-6 py-4">
 ...
 // Authentication Modal with simplified authentication flow (no IP tracking)
  ...
  const handleSendOtpClick = async () => {
    ...
    try {
      ...
      if (!normalizedPermittedEmails.includes(normalizedEmail)) {
        setLocalAuthError("This email is not authorized to access the dashboard");
        setIsProcessing(false);
      ...
      if (authMode === 'signup') {
        ...
        if (!memberError && memberData && memberData.signedUp) {
          setLocalAuthError("This email has already been registered. Please login instead");
          setIsProcessing(false);
        ...
        if (!memberError && memberData && !memberData.signedUp) {
          setLocalAuthError("This email is not registered yet. Please sign up first");
          setIsProcessing(false);
    ...
    const handleVerifyOtpClick = async () => {
      ...
      try {
        ...
        if (memberData.OTPExpiry && new Date(memberData.OTPExpiry) < new Date()) {
        ...
        if (!userError && existingUser && existingUser.length > 0 && 
          ...
          userId = existingUser[0].custom_auth_userID || existingUser[0].clerk_user_id;
          isNewUser = false;
        ...
        if (!sessionCheckError && existingSession && existingSession.length > 0) {
        ...
        if (authMode === 'signup' || avatarImage) {
          console.log("Updating PermittedMembers with avatar:", avatarImage ? "Avatar provided" : "No avatar");
          ...
          if (signupError) {
            console.error("Error updating signup status and avatar:", signupError);
          } else {
        ...
        // Show success message
        toast.success(isNewUser ? "Login successful! Please register your account." : "Login successful!");
      } catch (error) {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
        <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800 dark:text-gray-200">
          <h2 className="mb-4 text-xl font-semibold text-gray-800 dark:text-gray-200">
            {authMode === 'login' ? 'Login to Dashboard' : 'Sign Up for Dashboard'}
          </h2>
          <div className="space-y-4">
            {localAuthError && (
              ...
              <>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    >
                      {authMode === 'login' ? 'Need to sign up?' : 'Already have an account?'}
                    </button>
                  </div>
                  <button
                    ...
                    disabled={!localEmail || isProcessing || (authMode === 'signup' && !avatarImage)}
                  >
                    {isProcessing ? (
                      <>
      ...
      >
        ...
        <div className="mr-2 flex items-center">
          {(() => {
            ...
            const sessionData = localStorage.getItem('ecellSession');
            const avatar = sessionData ? JSON.parse(sessionData).avatar : null;
