...
function Page() {
  ...
  const [validationMessage, setValidationMessage] = useState("Initializing dashboard...");
...
return;
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'settings':
        return (
          <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
            ...
            <p className="mb-6">Configure dashboard settings and preferences.</p>
            {isSignedIn && !isRegistered && (
              <div className="mb-6 rounded-md bg-amber-50 p-4 dark:bg-amber-900/20">
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <h3 className="text-sm font-medium text-red-800 dark:text-amber-400">Registration Required</h3>
                    <div className="mt-2 text-sm text-red-700 dark:text-red-300">
                      <p>Your account needs to be registered to access all dashboard features.</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {isSignedIn && (
              <div className="mt-4">
                <h3 className="mb-2 text-lg font-medium">Account Registration</h3>
      ...
      default:
        return (
          <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
            <h2 className="mb-4 text-xl font-bold">Welcome to E-Cell Dashboard</h2>
  ...
  const LoadingScreen = () => (
    <div className="flex size-full flex-col items-center justify-center">
      ...
      <p className="text-lg font-medium text-gray-700 dark:text-gray-300">{validationMessage}</p>
    </div>
  );
    ...
    <div className="flex h-screen flex-col bg-gray-50 text-gray-800 dark:bg-gray-900 dark:text-gray-200">
      ...
      <header className="z-20 flex h-16 items-center justify-between border-b border-gray-200 bg-white px-4 dark:border-gray-700 dark:bg-gray-800">
        <div className="flex items-center">
          ...
          >
            {sidebarOpen && !isMobile ? <ChevronLeft className="size-6" /> : <Menu className="size-6" />}
          </button>
          <div className="flex items-center">
            <span className="text-xl font-bold text-indigo-600 dark:text-indigo-400">E-Cell</span>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside 
          className={`${
            ...
            !sidebarOpen && !isMobile ? 'translate-x-0' : !sidebarOpen ? '-translate-x-full' : 'translate-x-0'
          }`}
        >
            ...
            <nav className="space-y-1 p-2">
              {navItems.map((item) => (
                <button
                  ...
                  disabled={isValidating || (!isRegistered && item.id !== 'settings')}
                >
                  <div className={`${!sidebarOpen && !isMobile ? 'mr-0' : 'mr-3'}`}>
                    {item.icon}
                  </div>
        ...
        <main className={`flex-1 overflow-auto p-6 transition-all duration-300 ${sidebarOpen && !isMobile ? 'md:ml-0' : 'md:ml-0'}`}>
          {/* Status message */}
          ...
          <div className="mx-auto h-full max-w-7xl">
            {isValidating ? (
              ...
              <>
                <h1 className="mb-6 text-2xl font-bold">{navItems.find(item => item.id === activeTab)?.label || 'Dashboard'}</h1>
                {renderContent()}
              </>
            )}
          </div>
      ...
      {sidebarOpen && isMobile && (
        <div 
          className="fixed inset-0 z-0 bg-gray-600 bg-opacity-50 transition-opacity md:hidden" 
          onClick={toggleSidebar}
          aria-hidden="true"
        ></div>
      )}
