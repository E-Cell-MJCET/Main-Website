  ...
  const handleNext = async () => {
    ...
    if (sessionId) {
      localStorage.setItem(
        `${sessionId}_test_scores`,
        JSON.stringify(validTestScores)
      );
    }
    ...
    if (userData && userData.Profile_Data_Created) {
      ...
      const { error } = await supabase
        .from("Team")
        .update({ TestScores: validTestScores })
        .eq("custom_auth_userID", userId); // Update the row with the specified username
      alert("Test Scores updated successfully !");
      if (error) {
        console.log(error);
      }
    }
      ...
      >
        ...
        <div className="mb-6">
          <p className="mb-4 text-center text-gray-600">
            Add your standardized test scores, certifications, or assessment
            results to showcase your academic achievements.
          </p>
          {/* Test Scores Input */}
          <div className="mb-6">
              ...
              <div key={index} className="mb-4 rounded-lg border p-4 shadow-sm">
                <div className="mb-2 flex items-center justify-between">
                  <input
                    ...
                    value={item.title}
                    onChange={(e) =>
                      handleTestScoreChange(index, "title", e.target.value)
                    }
                ...
                <input
                  ...
                  value={item.score}
                  onChange={(e) =>
                    handleTestScoreChange(index, "score", e.target.value)
                  }
                  className="mb-2 w-full rounded-lg border border-gray-300 p-3"
                />
                <textarea
                  ...
                  onChange={(e) =>
                    handleTestScoreChange(index, "description", e.target.value)
                  }
                  className="mb-2 w-full rounded-lg border border-gray-300 p-3"
                  rows={3}
                />
            ...
            <div className="mt-8">
              ...
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {testScores
                    .filter((item) => item.title || item.score)
                    .map((item, index) => (
                      <div
                        key={index}
