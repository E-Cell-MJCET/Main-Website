...
const Step11Welcome = ({
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      const savedFeaturedItems = localStorage.getItem(
        `${sessionId}_featured_items`
      );
  ...
  const handleNext = async () => {
    ...
    if (sessionId) {
      localStorage.setItem(
        `${sessionId}_featured_items`,
        JSON.stringify(validFeaturedItems)
      );
    }
        ...
        <div className="mb-6">
          <p className="mb-4 text-center text-gray-600">
            ...
            <span className="mt-1 block text-sm text-indigo-600">
              Note: Maximum of 3 items will be displayed.
            </span>
          </p>
              ...
              <div key={index} className="mb-4 rounded-lg border p-4 shadow-sm">
                <div className="mb-2 flex items-center justify-between">
                  <input
                    ...
                    value={item.title}
                    onChange={(e) =>
                      handleFeaturedChange(index, "title", e.target.value)
                    }
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    handleFeaturedChange(index, "description", e.target.value)
                  }
                  className="mb-2 w-full rounded-lg border border-gray-300 p-3"
                  rows={4}
                />
