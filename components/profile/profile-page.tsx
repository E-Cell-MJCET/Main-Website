...
function CompletePage({ userData }: { userData: UserType }) {
  ...
  const handleEditClick = () => {
    ...
    if (user) {
      setEditedData({
        ...
        bio: user.about,
        badges: [], // Add appropriate badges if available
      ...
      const updatedExperience = user?.experience.map((exp) =>
        exp.id === editingExperienceId
          ? {
              ...exp,
              ...newExperience,
              endDate: isCurrentlyWorking ? "Present" : newExperience.endDate,
            }
          : exp
      );
      if (user) {
  ...
  const handleDeleteExperience = (id: string) => {
    if (user) {
      setUser({
        ...user,
        experience: user.experience.filter((exp) => exp.id !== id),
      });
    }
  };
        ...
        <div className="min-h-screen bg-black text-white">
          <div className="container mx-auto grid max-w-6xl grid-cols-1 gap-6 p-4 md:grid-cols-3">
            ...
            <LeftSection
              ...
              editedData={editedData || { name: "", bio: "" }}
